...
func build(tn *testnet.TestNet) error {
	...
	return util.LogError(helpers.AllNodeExecCon(tn, func(client ssh.Client, _ *db.Server, node ssh.Node) error {
		...
		return client.DockerExecdLog(node, "JAVA_OPTS=\"-Xmx512m -XX:+FlightRecorder\" gossip --sending --payloadSize 5000 --sendInterval 100 --numberOfMessages 10000 -n 0.0.0.0 -l 9000 -r 9001 -m /log.json " + peers + " 2>&1 | tee /output.log")
	}))
}
